...
diagnosis_distinct_YearMonth_index <- function(connection_Cosmos,project_string,IndexDateKey_Var="EventDateKey",
  ...
  FROM(SELECT DISTINCT def.PatientDurableKey, dtd.Value,dd.Year, dd.YearMonth
        ...
        WHERE (def.StartDateKey > ",filter_year,"0000) AND (def.StartDateKey < ",filter_year+1,"0000) AND 
            (def.StartDateKey >= (cohort.",IndexDateKey_Var,"-",LookBackInterval*10000,")) AND (def.StartDateKey < (cohort.",
...
diagnosis_distinct_Year_index <- function(connection_Cosmos,project_string,IndexDateKey_Var="EventDateKey",
                      LookBackInterval = 2,FollowUpInterval = 0,filter_year = numeric(),
                      by_letters = FALSE,alphabets=NULL){
  ...
  query <- paste0("SELECT all_dxcodes.PatientDurableKey, all_dxcodes.Value as ICD10_Value, all_dxcodes.Year,
  SUBSTRING(all_dxcodes.Value,1,3) AS Value_Grouper,
  SUBSTRING(all_dxcodes.Value,1,1) AS Value_Grouper2
  
  FROM(SELECT DISTINCT def.PatientDurableKey, dtd.Value,dd.Year
        FROM dbo.DiagnosisEventFact def
	        INNER JOIN ",project_string," cohort 
	          ON def.PatientDurableKey = cohort.PatientDurableKey
	        INNER JOIN dbo.DateDim dd
