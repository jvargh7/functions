problemlist_start_distinct_Range_index <- function(connection_Cosmos,project_string,IndexDateKey_Var="EventDateKey",
  ...
  FROM(SELECT DISTINCT plf.PatientDurableKey, dtd.Value,dd.Year, dd.YearMonth, plf.Chronic
        ...
        WHERE (plf.StartDateKey > ",filter_year,"0000) AND (plf.StartDateKey < ",filter_year+1,"0000) AND 
            (plf.StartDateKey >= (cohort.",IndexDateKey_Var,sign1,abs(LowerInterval)*10000,")) AND (plf.StartDateKey < (cohort.",
