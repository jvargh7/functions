...
# Check if problem list starts in specified year

problemlist <- function(connection_Cosmos,year,project_string,like_dx = NA_character_,by_letters = FALSE,alphabets=NULL){
  ...
  query <- paste0("SELECT all_dxcodes.PatientDurableKey, all_dxcodes.Value as ICD10_Value, ",date_vars_all_dxcodes,", all_dxcodes.Chronic,
  SUBSTRING(all_dxcodes.Value,1,3) AS Value_Grouper,
  SUBSTRING(all_dxcodes.Value,1,1) AS Value_Grouper2
  
  FROM(SELECT DISTINCT plf.PatientDurableKey, dtd.Value, ",date_vars_dd,", plf.Chronic
        FROM dbo.ProblemListFact plf
	        INNER JOIN (
            SELECT DISTINCT(cohort.PatientDurableKey) as PatientDurableKey
            FROM ",project_string," cohort 
          ) distinct_cohort
	         ON plf.PatientDurableKey = distinct_cohort.PatientDurableKey
	        INNER JOIN dbo.DateDim dd
	         ON plf.StartDateKey = dd.DateKey ",
          selected_dx,
        " WHERE (plf.StartDateKey > ",year,"0000) AND (plf.StartDateKey < ",year+1,"0000)
